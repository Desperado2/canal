...
if [ -f $base/bin/k2s.pid ] ; then
	echo "found k2s.pid , Please run stop.sh first ,then startup.sh" 2>&2
    exit 1
fi
...
$JAVA $JAVA_OPTS $JAVA_DEBUG_OPT $CANAL_OPTS -classpath .:$CLASSPATH com.alibaba.otter.canal.k2s.K2SApplication 1>>/dev/null 2>&1 &
echo $! > $base/bin/k2s.pid
