  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="所属主机" min-width="200" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.taskNodeServer !== null">
            {{ scope.row.taskNodeServer.name }}
          </span>
          <span v-else>-</span>
        </template>
      </el-table-column>
...
<script>
import { getCanalInstances, deleteCanalInstance, instanceStatus } from '@/api/consumerCanalInstance'
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      this.$router.push('/consumerServer/canalInstance/add')
    },
    handleUpdate(row) {
      this.$router.push('/consumerServer/canalInstance/modify?id=' + row.id)
    },
    handleDelete(row) {
    ...
    handleLog(row) {
      if (row.taskNodeServer === null) {
        this.$message({ message: '当前Instance不是启动状态，无法查看日志', type: 'warning' })
        return
      }
      this.$router.push('canalInstance/log?id=' + row.id + '&nodeId=' + row.taskNodeServer.id)
    }
  }
}
</script>
